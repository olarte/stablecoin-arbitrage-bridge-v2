...
const StableArbBridge = () => {
  ...
  const fetchOpportunities = async () => {
    try {
      const response = await fetch(`${API_BASE}/swap/scan-opportunities?includeCelo=true&includeTriangular=true`);
      const data = await response.json();
      if (data.success) {
  ...
  const executeTrade = async (opportunity, dryRun = true) => {
    try {
      ...
      if (result.success) {
        alert(`${dryRun ? 'Simulation' : 'Trade'} completed successfully! Profit: ${result.data.summary?.actualProfit}%`);
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <div className={`w-2 h-2 rounded-full ${walletStatus?.portfolio?.readyForTrading ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
                <span className="text-sm">
                  {walletStatus?.portfolio?.readyForTrading ? 'Ready' : 'Setup Required'}
                </span>
              </div>
              <button
                onClick={() => {fetchWalletStatus(); fetchOpportunities();}}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-gray-800 rounded-lg p-6">
                ...
                <div className="text-sm text-gray-400">
                  Across {Object.keys(walletStatus?.portfolio?.breakdown || {}).length} chains
                </div>
              </div>
              ...
              <div className="bg-gray-800 rounded-lg p-6">
                ...
                <div className="text-sm text-gray-400">
                  {opportunities.filter(o => o.confidence === 'HIGH').length} high confidence
                </div>
              </div>
              ...
              <div className="bg-gray-800 rounded-lg p-6">
                ...
                <div className="text-2xl font-bold text-purple-400 mb-2">
                  {opportunities.length > 0 ? `${Math.max(...opportunities.map(o => o.spread)).toFixed(2)}%` : '0.00%'}
                </div>
                <div className="text-sm text-gray-400">
                  {opportunities.length > 0 ? opportunities[0]?.pair : 'No opportunities'}
                </div>
              </div>
            </div>
              ...
              {opportunities.slice(0, 5).map((opp, index) => (
                <div key={index} className="flex items-center justify-between py-3 border-b border-gray-700 last:border-b-0">
                  <div>
                    ...
                    <div className="text-sm text-gray-400">
                      {opp.chains?.join(' → ') || opp.direction} • {opp.confidence} confidence
                    </div>
                  </div>
                  <div className="text-right">
                    <div className={`font-bold ${opp.spread > 1 ? 'text-green-400' : 'text-yellow-400'}`}>
                      {opp.spread}%
                    </div>
                    <div className="text-sm text-gray-400">
                      ~${((opp.recommendedAmount || 50) * opp.spread * 0.007).toFixed(2)}
                    </div>
                  </div>
                </div>
              ))}
                ...
                <div key={index} className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    <div>
                      ...
                      <div className="font-medium">${opp.recommendedAmount || 50}</div>
                    </div>
                    <div>
                      <div className="text-sm text-gray-400">Est. Profit</div>
                      <div className="font-medium text-green-400">{opp.estimatedProfit}</div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className={`font-medium ${
                        ...
                        opp.riskLevel === 'MEDIUM' ? 'text-yellow-400' : 'text-red-400'
                      }`}>
                  ...
                  {opp.specialFeatures && (
                    <div className="mt-4 pt-4 border-t border-gray-700">
                      <div className="text-sm text-gray-400">Special Features:</div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-6">
              {['ethereum', 'sui', 'celo'].map(chain => {
                ...
                return (
                  <div key={chain} className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center">
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          {Object.entries(wallet.balances || {}).map(([token, balance]) => {
                            ...
                            return (
                              <div key={token} className="text-center p-3 bg-gray-700 rounded-lg">
                    ...
                    {!wallet?.connected && (
                      <div className="text-gray-400">
                        ...
                        <p className="text-sm">
                          Configure your {chainName.toUpperCase()}_TEST_PRIVATE_KEY in the .env file.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            {walletStatus?.portfolio && (
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="text-center">
                    <div className={`text-2xl font-bold ${
                      walletStatus.portfolio.readyForTrading ? 'text-green-400' : 'text-yellow-400'
                    }`}>
                      {walletStatus.portfolio.readyForTrading ? 'Ready' : 'Setup'}
                    </div>
        ...
        {activeTab === 'trading' && (
          <div className="space-y-6">
            ...
            <div className="bg-yellow-600 border border-yellow-500 rounded-lg p-4 flex items-center">
              <AlertCircle className="w-5 h-5 mr-2" />
              <span>This is testnet trading only. No real money is at risk.</span>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Best Opportunities */}
              <div className="bg-gray-800 rounded-lg p-6">
                <h3 className="text-lg font-semibold mb-4">Best Opportunities</h3>

                {opportunities.slice(0, 3).map((opp, index) => (
                  <div key={index} className="p-4 bg-gray-700 rounded-lg mb-4 last:mb-0">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {tradingEnabled && (
                        <button
                          onClick={() => {
                            if (window.confirm('Execute real trade?')) executeTrade(opp, false);
                          }}
              ...
              <div className="bg-gray-800 rounded-lg p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className={`p-3 rounded-lg ${
                      ...
                      <div className="flex items-center">
                        ...
                        {tradingEnabled ? 'Real Trading Enabled' : 'Simulation Mode Only'}
                      </div>
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm text-gray-400 mb-2">Safety Limits</label>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-gray-700 p-3 rounded-lg">
                        <div className="text-sm text-gray-400">Max Trade</div>
                        <div className="font-medium">
                          ${walletStatus?.trading?.safetyLimits?.maxTradeUSD || 100}
                        </div>
                      </div>
                      <div className="bg-gray-700 p-3 rounded-lg">
                        <div className="text-sm text-gray-400">Min Profit</div>
                        <div className="font-medium">
                          {walletStatus?.trading?.safetyLimits?.minProfitPercent || 0.3}%
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm text-gray-400 mb-2">Supported Chains</label>
                    <div className="flex space-x-2">
                      {['Ethereum', 'Sui', 'Celo'].map(chain => (
                        <span
                          key={chain}
            ...
            <div className="grid gap-6">
              ...
              <div className="bg-gray-800 rounded-lg p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className={`text-2xl font-bold ${
                      walletStatus?.trading?.enabled ? 'text-green-400' : 'text-gray-400'
                    }`}>
